...
export class IssuesComponent {
  ...
  getIssues() {
    this.http.get<any[]>('http://localhost:3000/api/event/issues').subscribe({
      next: (data) => {
        this.issues = data; // Assign fetched data to opinions array
      },
      error: (error) => {
        console.error('Error fetching opinions:', error); // Handle errors
      },
  ...
  getAllOpinions() {
    this.http.get<any[]>('http://localhost:3000/api/opinion').subscribe({
      next: (data) => {
        this.opinions = data; // Assign fetched data to opinions array
      },
      error: (error) => {
        console.error('Error fetching opinions:', error); // Handle errors
      },
